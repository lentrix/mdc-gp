...
if(isset($_POST['activate'])) {
    ...
    $st1 = $pdo->prepare("SELECT * FROM `gp-users` WHERE idnum=?");
    $st1->execute([$idnum]);

    $acct = $st1->fetch(PDO::FETCH_OBJ);

    if($acct) {
        $errors[] = "The account of $idnum already exists. 
            Please contact the registrar's office in case you 
